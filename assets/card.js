...
if (!localStorage.getItem('givenDate')) {
  // Załóż, że birthdayDate istnieje (czyli wcześniej poprawnie sparsowano datę urodzenia)
  const givenDateObj = new Date(birthdayDate);
  givenDateObj.setFullYear(givenDateObj.getFullYear() + 18); // Dodaj 18 lat

  const givenDate = givenDateObj.toLocaleDateString("pl-PL", options);
  localStorage.setItem('givenDate', givenDate);

  // Dodaj 10 lat do daty wydania (czyli 18 + 10 = 28 lat od urodzin)
  const expiryDateObj = new Date(givenDateObj);
  expiryDateObj.setFullYear(expiryDateObj.getFullYear() + 10);
  const expiryDate = expiryDateObj.toLocaleDateString("pl-PL", options);
